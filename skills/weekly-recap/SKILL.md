...
```python
import datetime
today = datetime.date.today()  # computed at runtime — never hardcoded
# For each item with a due date:
# due = datetime.date.fromisoformat("YYYY-MM-DD")
# if due < today: skip this item entirely
```

If a due date is present and has already passed, exclude the item regardless of its status.
